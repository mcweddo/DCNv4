...
TORCH_CUDA_ARCH_LIST="5.0;6.0;7.0;7.5;8.0;8.6;8.9"
...
if [[ $CUDA_VERSION == "12.1" || $CUDA_VERSION == "12.4" || $CUDA_VERSION == "12.8"]]; then
    ...
    if [[ -z "$PYTORCH_EXTRA_INSTALL_REQUIREMENTS" ]]; then
        echo "Bundling with cudnn and cublas."
        DEPS_LIST+=(
            "/usr/local/cuda-8.0/lib/libcudnn_adv_infer.so.8"
            "/usr/local/cuda-8.0/lib/libcudnn_adv_train.so.8"
            "/usr/local/cuda-8.0/lib/libcudnn_cnn_infer.so.8"
            "/usr/local/cuda-8.0/lib/libcudnn_cnn_train.so.8"
            "/usr/local/cuda-8.0/lib/libcudnn_ops_infer.so.8"
            "/usr/local/cuda-8.0/lib/libcudnn_ops_train.so.8"
            "/usr/local/cuda-8.0/lib/libcudnn_heuristic.so.8"
            "/usr/local/cuda-8.0/lib/libcudnn.so.8"
